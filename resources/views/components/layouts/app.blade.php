...
<body class="text-slate-800">
    <div class="min-h-screen lg:grid lg:grid-cols-[220px_1fr]">
        <aside class="hidden lg:sticky lg:top-0 lg:flex h-screen flex-col overflow-hidden border-r border-white/60 bg-ink text-white">
            ...
            <nav class="flex-1 space-y-2 overflow-y-auto px-4 py-5 text-sm">
                <a href="{{ route('dashboard') }}" class="block rounded-lg px-3 py-2 transition {{ request()->routeIs('dashboard') ? 'bg-white/20 font-semibold text-aqua' : 'hover:bg-white/10' }}">Dashboard</a>
            ...
            <div class="shrink-0 border-t border-white/20 bg-ink px-4 py-4">
                <p class="mb-3 text-xs text-white/80">Login sebagai {{ auth()->user()?->role }}</p>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="w-full rounded-lg bg-white/15 px-3 py-2 text-sm font-medium text-white hover:bg-white/25">Logout</button>
                </form>
            </div>
        </aside>
